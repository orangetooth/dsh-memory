...
import { collectResponse, extractJsonObject, generateOptions, parseCallArguments, PHASE1_REASONING, resolveStageRoute, } from './llm.js';
...
function rolloutFrontmatter(header, part, baseSlug) {
    const lines = [
        '<!--',
        `  session: ${header.id}`,
...
export class Phase1Runner {
     ...
     */
    async callExtract(header, route, cfg, transcript, delta) {
        const { llm } = this.deps;
        const meta = {
            sessionId: header.id,
            ...(header.cwd === undefined ? {} : { cwd: header.cwd }),
        ...
        const extract = async (text) => {
            const response = await collectResponse(llm, generateOptions(route, PHASE1_SYSTEM, phase1User(meta, text, delta), cfg.phase1MaxTokens, undefined, [PHASE1_TOOL]), 80_000);
            let raw;
