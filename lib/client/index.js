...
function MemorySettings() {
    ...
    if (payload === null || form === null) {
        return el('div', { className: 'dm-section' }, el('p', { className: 'dm-intro' }, notice !== null ? notice.text : '加载中…'));
    }
    const set = (key, value) => {
    ...
    }, el('option', { value: '' }, '— 使用部署默认模型 —'), knownProviders.map(entry => el('option', { key: entry.id, value: entry.id }, entry.name || entry.id)), providerKnown ? null : el('option', { value: '__custom__' }, `${provider}（自定义）`));
    ...
    const modelControl = provider === ''
        ...
        : knownModels.length > 0 && !manualModel
            ...
            }, el('option', { value: '' }, '— 请选择 —'), knownModels.map(entry => el('option', { key: entry.id, value: entry.id }, entry.name || entry.id)))
            : el('input', {
                className: 'dm-input', value: String(form.model ?? ''), disabled: busy,
                placeholder: '手动输入模型 ID',
                onChange: event => set('model', event.target.value),
            });
    const failureLines = (Array.isArray(payload.recentFailures) ? payload.recentFailures : []).map(failure => el('div', { className: 'dm-fail', key: failure.session }, el('p', { className: 'dm-dimmed' }, `${shortId(failure.session)} · ${fmtTime(failure.at)}`), el('p', { className: 'dm-warn' }, failure.error || '未知错误')));
    return el('div', { className: 'dm-section' }, el('p', { className: 'dm-title' }, '长期记忆'), el('p', { className: 'dm-intro' }, '把会话历史自动提炼为长期记忆：Phase 1 逐会话提取（产出回顾与原始记忆），Phase 2 由内置受限 agent 渐进检索并整合进 MEMORY.md 与 memory_summary.md；无需额外服务。'), el('div', { className: 'dm-card' }, el('div', { className: 'dm-row' }, el('span', { className: 'dm-label' }, '记忆库：'), el('span', { className: 'dm-dimmed', style: { wordBreak: 'break-all' } }, payload.root)), el('div', { className: 'dm-row' }, el('span', { className: 'dm-label' }, '记忆父会话：'), el('span', { className: 'dm-dimmed', style: { wordBreak: 'break-all' } }, payload.memoryParentSessionId == null
        ? '正在创建…'
        : `长期记忆（后台整合） · ${payload.memoryParentSessionId}`)), el('div', { className: 'dm-grid' }, stat('已处理会话', `${payload.counts.done} 条记忆 / ${payload.counts.noop} 跳过 / ${payload.counts.failed} 失败`), stat('会话回顾', `${payload.stats.rollouts} 份`), stat('固化的 skills', `${payload.stats.skills} 个`), stat('ad hoc 笔记', `${payload.stats.notes} 条`), stat('MEMORY.md', fmtChars(payload.stats.memoryChars)), stat('memory_summary.md', fmtChars(payload.stats.summaryChars)), stat('待整合 raw', fmtChars(payload.stats.rawChars)), stat('整合冷却', pipeline.cooldownRemainingMs > 0 ? `剩余 ${Math.ceil(pipeline.cooldownRemainingMs / 3_600_000)} 小时` : '已就绪')), el('p', { className: 'dm-dimmed' }, `Phase 1：${fmtTime(pipeline.lastPhase1At)} · Phase 2：${fmtTime(pipeline.lastPhase2At)}`), pipeline.phase2Error === undefined
        ? null
        : el('p', { className: 'dm-warn' }, `上次整合失败：${pipeline.phase2Error}`), el('div', { className: 'dm-row' }, el('button', { className: 'dm-btn primary', disabled: busy, onClick: () => { void runPhase1(); } }, '立即提取'), el('button', { className: 'dm-btn primary', disabled: busy, onClick: () => { void call('run-phase2', {}, '整合完成。'); } }, '立即整合'), payload.counts.failed > 0
        ...
        : null, el('button', { className: 'dm-btn secondary', disabled: busy, onClick: () => { void call('get-state').then(() => setNotice({ kind: 'ok', text: '已刷新。' })).catch(() => { }); } }, '刷新'), el('span', { className: 'dm-dimmed' }, `当前路由：${routeLabel}`)), el('p', { className: 'dm-dimmed' }, stageReasoning)), failureLines.length > 0
        ? el('div', { className: 'dm-card' }, el('div', { className: 'dm-label' }, '最近失败的会话（按退避自动重试；错误含实际使用的模型路由）'), ...failureLines)
        : null, el('div', { className: 'dm-card' }, el('label', { className: 'dm-check' }, el('input', {
        type: 'checkbox', checked: form.enabled === true, disabled: busy,
        onChange: event => set('enabled', event.target.checked),
    }), '启用长期记忆管道（提取、整合与摘要注入）'), el('div', { className: 'dm-grid' }, num('会话结束后防抖（分钟）', 'idleDebounceMinutes', '轮次结束后的静默时长，默认 3 分钟'), num('整合冷却（小时）', 'cooldownHours', '两次自动整合的最小间隔，默认 6 小时'), num('每轮处理会话数', 'maxRolloutsPerRun', '每次管道运行提取的会话数上限'), num('注入摘要上限（字符）', 'maxSummaryChars', 'memory_summary.md 注入 system prompt 的截断上限'), num('提取输出 token 上限', 'phase1MaxTokens', '默认 16384，容纳会话回顾与原始记忆'), num('整合输出 token 上限', 'phase2MaxTokens', '默认 65535；为 high 思考与完整结构化结果保留足够空间')), el('div', { className: 'dm-grid' }, el('div', { className: 'dm-field' }, el('div', { className: 'dm-label' }, '记忆管道模型提供方（可选）'), providerSelect, el('p', { className: 'dm-dimmed' }, '留空使用部署默认模型；列表来自 设置 → 模型 中已注册的提供方')), el('div', { className: 'dm-field' }, el('div', { className: 'dm-label' }, '记忆管道模型（可选）'), modelControl, provider !== ''
        ? el('label', { className: 'dm-check' }, el('input', {
        ...
        : el('p', { className: 'dm-dimmed' }, '选择提供方后可下拉选择模型'))), el('div', { className: 'dm-row' }, el('button', { className: 'dm-btn primary', disabled: busy, onClick: save }, '保存配置'))), notice === null ? null : el('p', { className: notice.kind === 'ok' ? 'dm-ok' : 'dm-error' }, notice.text), payload.storage === 'ok'
        ? el('p', { className: 'dm-dimmed' }, '配置与处理进度已持久化到 DSH 存储（storage: json），重启后自动恢复。')
        : el('p', { className: 'dm-warn' }, `持久化不可用（${payload.storageError || 'storage json 后端缺失'}）：进度与配置仅在本次运行期间有效。`), el('p', { className: 'dm-dimmed' }, '会话内用法：助手可调用 memory_list / memory_read / memory_search / memory_add（仅在用户明确要求时）工具查阅与更新记忆。'));
}
export const inject = [];
export function apply(ctx) {
    ...
    slots.inject('settings.section', () => slots.register({ name: 'settings.section', id: 'dsh-memory', order: 30, label: '长期记忆' }, () => React.createElement(MemorySettings)));
    ctx.effect(() => () => {
        style.remove();
    });
}
