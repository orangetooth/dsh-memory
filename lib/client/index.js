...
function MemorySettings() {
    ...
    return el('div', { className: 'dm-section' }, el('p', { className: 'dm-title' }, '长期记忆'), el('p', { className: 'dm-intro' }, '把会话历史自动提炼为长期记忆：Phase 1 逐会话提取（产出回顾与原始记忆），Phase 2 全局整合进 MEMORY.md 与 memory_summary.md；摘要常驻注入新会话，工具 memory_list/read/search/add 按需检索。'), el('div', { className: 'dm-card' }, el('div', { className: 'dm-row' }, el('span', { className: 'dm-label' }, '记忆库：'), el('span', { className: 'dm-dimmed', style: { wordBreak: 'break-all' } }, payload.root)), el('div', { className: 'dm-grid' }, stat('已处理会话', `${payload.counts.done} 条记忆 / ${payload.counts.noop} 跳过 / ${payload.counts.failed} 失败`), stat('会话回顾', `${payload.stats.rollouts} 份`), stat('固化的 skills', `${payload.stats.skills} 个`), stat('ad hoc 笔记', `${payload.stats.notes} 条`), stat('MEMORY.md', fmtChars(payload.stats.memoryChars)), stat('memory_summary.md', fmtChars(payload.stats.summaryChars)), stat('待整合 raw', fmtChars(payload.stats.rawChars)), stat('整合冷却', pipeline.cooldownRemainingMs > 0 ? `剩余 ${Math.ceil(pipeline.cooldownRemainingMs / 3_600_000)} 小时` : '已就绪')), el('p', { className: 'dm-dimmed' }, `Phase 1：${fmtTime(pipeline.lastPhase1At)} · Phase 2：${fmtTime(pipeline.lastPhase2At)}`), pipeline.phase2Error === undefined
        ? null
        : el('p', { className: 'dm-warn' }, `上次整合失败：${pipeline.phase2Error}`), el('div', { className: 'dm-row' }, el('button', { className: 'dm-btn primary', disabled: busy, onClick: () => { void call('run-phase1', {}, '提取完成。'); } }, '立即提取'), el('button', { className: 'dm-btn primary', disabled: busy, onClick: () => { void call('run-phase2', {}, '整合完成。'); } }, '立即整合'), el('button', { className: 'dm-btn secondary', disabled: busy, onClick: () => { void call('get-state').then(() => setNotice({ kind: 'ok', text: '已刷新。' })).catch(() => { }); } }, '刷新'), el('span', { className: 'dm-dimmed' }, `当前路由：${routeLabel}`))), failureLines.length > 0
