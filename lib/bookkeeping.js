...
export class MemoryStateStore {
    ...
    recoverRunningBefore(horizon, error) {
        let recovered = 0;
    ...
    claimDone(id, slug, lastSeq, parts) {
        const previous = this.state.processed[id];
        this.state.processed[id] = {
            status: 'done',
            at: this.now(),
    ...
    get memoryParentSessionId() {
        return this.state.memoryParentSessionId;
    }
    setMemoryParentSessionId(id) {
        if (this.state.memoryParentSessionId === id)
            return;
