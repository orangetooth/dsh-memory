...
- 🧵 **专用 memory parent**：插件创建并跨重启恢复一个工作目录固定为记忆库的空白
  顶层会话，所有 Phase 2 child 只挂在它下面；不会再借用最近活跃的业务会话，也不会
  继承业务工作区的 preset、项目说明或父会话历史。WebUI 中可按「长期记忆（后台整合）」查看。
...
- 🔁 **可靠调度**：每个会话一个租约（KV 持久化），重启不重复处理、不重复整合；
  失败带退避重试；孤儿租约自动回收。
- ⚙️ **设置页**：设置 → 长期记忆 中查看统计与专用父会话 ID、手动触发提取/整合、调整冷却与模型路由。
